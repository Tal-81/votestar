...
class UserAdmin(BaseUserAdmin):
    ...
    def delete_model(self, request, obj):
        ...
        if obj.is_superuser:
            self.message_user(
                request,
                f'Cannot delete admin account "{obj.email}". Admin accounts are protected.',
                level=messages.ERROR,
            )
            return  # Do NOT call super() — abort the deletion
    ...
    def delete_queryset(self, request, queryset):
        ...
        if protected.exists():
            emails = ', '.join(protected.values_list('email', flat=True))
            self.message_user(
                request,
                f'Cannot delete admin account(s): {emails}. Admin accounts are protected.',
                level=messages.ERROR,
            )
            # Only delete the non-superuser accounts from the selection
